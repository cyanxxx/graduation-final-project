...
export default class NavItem extends Component<Props,States> {
    ...
    renderMenu() {
        ...
        return (
            <div className="menuBox" style={{ display: 'flex' }}>
                <div className="lm">
                    {sort!.map((el) => {
                        return <div className={`sort ${currentSort === el.id ? 'active' : ''}`} key={el.id} onClick={() => {
                            this.setState({
                                subSort: el.subSort,
                                currentSort: el.id
                            })
                ...
                <div className="rd" style={{ background: '#eee' }}>
                    {subSort.map((item) => {
                        return <div className='detailSort' key={item.id} data-id={item.id} onClick={() => {
    ...
    renderLocation() {
        ...
        return (
            <div className="menuBox" style={{ display: 'flex' }}>
                <div className="lm">
                    {location!.map((el) => {
                        return <div className={`sort ${currentLocation === el.name ? 'active' : ''}`} key={el.name} onClick={() => {
                            this.setState({
  ...
  render() {
    ...
    return (
      <div>
            { filterSort === filter.sort ? this.renderMenu() : this.renderLocation()}
      </div>
    )
  }
}
