...
export default class TripForm extends Component<componentProps,State> {
  ...
  create = async () => {
      ...
      const result = await this.props.core.db.post('/item', data as APIPost['/item']['req']) as APIPost['/item']['res']
      if (!result) {
          Alert({ message: '请填入地点' })
      }
      
      this.props.core.history.push(ROUTE.tripList, { id: this.props.location.state.id })
  }
  render() {
      ...
      <Card>
        <div className="field">
            ...
            <div className="control">
                <input className="input" type="text" placeholder="输入目的地" value={loaction} onChange={(ev)=>this.setState({loaction: ev.target.value})} required/>
            </div>
        </div>
        <div className="field">
        ...
        <div className="field">
            ...
            <div className="control">
                <input className="input" type="text" placeholder="备注" value={info} onChange={(ev)=>this.setState({info: ev.target.value})}/>
            </div>
        </div>
            <div className="field">
                <p className="control">
