...
export default class CurrentWeather extends React.Component<Props, State> {
  ...
  cityHandle = async (e: React.MouseEvent<HTMLAnchorElement, MouseEvent>) => {
    ...
    const json = (await this.getWeather(target.getAttribute(
      'data-name',
    ) as string)) as APIGet['/weather']['res'];
        ...
        <Card>
          ...
          <div className="info">
            ...
            <p>
              日落：
              {sunset
                ? new Date(sunset! * 1000).getHours() + ':' + new Date(sunset! * 1000).getMinutes()
                : 0}
            </p>
    ...
    return (
      <figure>
        ...
        <div
          ...
          className="info"
        >
          <p onClick={this.switchCity} data-name={city}>
            {city === 'Guangzhou' ? '广州' : city}
          </p>
          {/* <p>{dec}</p> */}
          <p>{temp}°C</p>
        ...
        <Modal
          render={open}
          title={'天气'}
          pannelHandle={() => this.setState(prev => ({ open: !prev.open }))}
        >
          <ChoiceWearher
            style={{ height: '50vh', background: '#fff' }}
            onChoice={this.cityHandle}
