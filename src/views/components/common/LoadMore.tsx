...
export default class LoadMore extends Component<Props,States> {
  ...
  reachBottom() {
    ...
    if (this.contentBottom && getOffset(this.contentBottom!).top < window.innerHeight + window.pageYOffset) {
      getDataHandle(curPage + 1 )
    }
  }
  render() {
    return (
        <div className="line" ref={((bottom) => this.contentBottom = bottom)}></div>
    )
  }
}
