...
export default class Menu extends Component<Props, States> {
  ...
  async getNewData(p?: number, category?: category) {
    ...
    if(!this.loading) {
      ...
      const data = await this.props.core.db.get('/restaurant/list', { id: query, page: page }) as APIGet['/restaurant/list']['res']
      this.loading = false
      this.setState({
        canNext: !!data.next,
  ...
  async getSortData() {
    const data = await this.props.core.db.get('/restaurant/sort', undefined) as APIGet['/restaurant/sort']['res']
    this.setState({
      sort: data
    })
  }
  async getLocationData() {
    const data = await this.props.core.db.get('/restaurant/location', undefined) as APIGet['/restaurant/location']['res']
    this.setState({
      location: data
    })
  }
  ...
  searchHandle = async (value) =>{
    const data = await this.props.core.db.get('/restaurant/list', { search: value })
    if(data){
      this.setState({
        data: data.results,
        canNext: !!data.next,
