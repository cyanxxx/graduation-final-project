...
export class DB {
  ...
  constructor() {
    this.protocol = 'http';
    this.host = '139.155.25.23'; // for build
  ...
  public _parseURL(_path: string, obj?: URLQuery) {
    const path = obj ? parsePath(_path, obj) : _path + '/';
    console.log(path);
    return `${this.protocol}://${this.host}:${this.port}${path}`;
  }
  public async get<Path extends keyof APIGet>(
    _path: Path,
    query: APIGet[Path]['req'],
  ): Promise<APIGet[Path]['res'] | null> {
    try {
  ...
  public async post<Path extends keyof APIPost>(
    _path: Path,
    data: APIPost[Path]['req'],
  ): Promise<APIPost[Path]['res'] | null> {
    try {
